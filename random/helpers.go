...
func Init() {
	...
	if seed := os.Getenv("SYSTEMQUEST_RANDOM_SEED"); seed != "" {
...
func RandomFloat64(min, max float64) float64 {
	...
	if bigMin == nil {
		panic("Systemquest Internal Error - min boundary is not finite")
	}

	bigMax := new(big.Rat).SetFloat64(max)
	if bigMax == nil {
		panic("Systemquest Internal Error - max boundary is not finite")
	}
