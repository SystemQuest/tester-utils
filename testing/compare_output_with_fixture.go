...
func CompareOutputWithFixture(t *testing.T, testerOutput []byte, normalizeOutputFunc func([]byte) []byte, fixturePath string) {
	shouldRecordFixture := os.Getenv("SYSTEMQUEST_RECORD_FIXTURES") == "true"
	...
	if err != nil {
		if os.IsNotExist(err) {
			if shouldRecordFixture {
				...
				t.Errorf("Fixture file %s does not exist. To create a new one, use SYSTEMQUEST_RECORD_FIXTURES=true", fixturePath)
				return
			}
		}
	...
	os.Stdout.Write([]byte("\n\nRe-run this test with SYSTEMQUEST_RECORD_FIXTURES=true to update fixtures\n\n"))
	t.FailNow()
}
